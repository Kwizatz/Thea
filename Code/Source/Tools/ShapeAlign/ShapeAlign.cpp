...
{
  THEA_CONSOLE << "";
  THEA_CONSOLE << "ShapeAlign: Rigid shape registration using Iterative Closest Point (ICP)";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "This tool computes an optimal rigid transformation (rotation + translation,";
  THEA_CONSOLE << "optionally with uniform scaling) to align a source shape to a target shape.";
  THEA_CONSOLE << "It outputs a 3x4 affine transformation matrix (row-major) that transforms";
  THEA_CONSOLE << "the source shape to best match the target.";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "Usage: " << argv[0] << " [<options>] <from> <to> [<output>]";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "Arguments:";
  THEA_CONSOLE << "  <from>    :  Source shape to be aligned (moved/transformed)";
  THEA_CONSOLE << "  <to>      :  Target shape to align to (remains fixed)";
  THEA_CONSOLE << "  <output>  :  Output file for the transformation matrix (optional,";
  THEA_CONSOLE << "               defaults to stdout). Use .txt extension for a list of";
  THEA_CONSOLE << "               multiple source shapes to align.";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "Supported input formats:";
  THEA_CONSOLE << "  Meshes (auto-sampled to point clouds):";
  THEA_CONSOLE << "    .obj   - Wavefront OBJ";
  THEA_CONSOLE << "    .3ds   - Autodesk 3DS";
  THEA_CONSOLE << "    .ply   - Stanford PLY";
  THEA_CONSOLE << "    .off   - Object File Format (with faces)";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "  Point clouds (one point per line: x y z):";
  THEA_CONSOLE << "    .off   - OFF format (vertices only, no faces)";
  THEA_CONSOLE << "    .arff  - ARFF format (first 3 attributes are x, y, z)";
  THEA_CONSOLE << "    .pts/.xyz/other - Plain text (whitespace-separated x y z per line)";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "  Shape lists:";
  THEA_CONSOLE << "    .txt   - Text file with one shape path per line (for batch processing)";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "Options:";
  THEA_CONSOLE << "  -n <#mesh-samples>  :  Approximate #points to sample from mesh (default: 5000)";
  THEA_CONSOLE << "  -s                  :  Normalize shape scales (match average distance from centroid)";
  THEA_CONSOLE << "  -t x y z            :  Initial location for centroid of <from>";
  THEA_CONSOLE << "  -c                  :  Align shape centroids before starting ICP";
  THEA_CONSOLE << "  -x                  :  Test over various axis-aligned rotations (24 orientations)";
  ...
  THEA_CONSOLE << "  -u x y z            :  Up vector (e.g., 0 1 0 for Y-up, 0 0 1 for Z-up)";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "Output format:";
  THEA_CONSOLE << "  The transformation matrix is output as 12 space-separated values representing";
  THEA_CONSOLE << "  a 3x4 row-major affine matrix [R|t] where R is 3x3 rotation and t is translation:";
  THEA_CONSOLE << "    r00 r01 r02 tx r10 r11 r12 ty r20 r21 r22 tz";
  THEA_CONSOLE << "";
  THEA_CONSOLE << "Examples:";
  THEA_CONSOLE << "  " << argv[0] << " source.obj target.obj";
  THEA_CONSOLE << "  " << argv[0] << " -s -c source.ply target.ply transform.txt";
  THEA_CONSOLE << "  " << argv[0] << " -x -u 0 1 0 source.off target.off";
  THEA_CONSOLE << "";
  return -1;
}

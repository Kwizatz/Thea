...
static Map<ITexture::Format const *, bool> _supportedRenderBufferFormat;
...
bool
GlCaps::supportsTexture(ITexture::Format const * fmt)
{
  // First, check if we've already tested this format
  Map<ITexture::Format const *, bool>::iterator existing = _supportedTextureFormat.find(fmt);
  if (existing == _supportedTextureFormat.end())
  {
    bool supportsFormat = false;

    if (!(fmt->isFloatingPoint() && !THEA_GL_SUPPORTS(ARB_texture_float)))
    {
...
bool
GlCaps::supportsRenderBuffer(ITexture::Format const * fmt)
{
  // First, check if we've already tested this format
  Map<ITexture::Format const *, bool>::iterator existing = _supportedRenderBufferFormat.find(fmt);
  if (existing == _supportedRenderBufferFormat.end())
  {
    bool supportsFormat = false;

    if (THEA_GL_SUPPORTS(EXT_framebuffer_object) && !(fmt->isFloatingPoint() && !THEA_GL_SUPPORTS(ARB_texture_float)))
    {
